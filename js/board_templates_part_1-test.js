	...
	if (assignedList.length > 0) {
		if (assignedList.length <= 3) {
			for (let i = 0; i < assignedList.length; i++) {
				...
				let initials = assignedList[i]['firstSecondLetter'];
				let colorIndex = assignedList[i]['colorIndex'];
				let assignToTitle = name;
				console.log('sdf', colorIndex);
				document.getElementById(divId).innerHTML += /*html*/ `
                <div class='contributorsLogo' title='${assignToTitle}' style='background-color: ${colorIndex}'>
                    <span>${initials}</span>
                </div>`;
			}
		} else {
